...
class KcAgent < Formula
  ...
  def install
    libexec.install "kc-agent" => "kc-agent-bin"

    (bin/"kc-agent").write <<~SHELL
      #!/usr/bin/env bash
      set -euo pipefail

      # Optimize client environment: disable in-cluster kagenti DNS stalls
      export KAGENTI_CONTROLLER_URL="${KAGENTI_CONTROLLER_URL:-none}"

      # Configure origin flags for local console if not already provided
      DEFAULT_ORIGINS="http://localhost:8080,http://127.0.0.1:8080"
      if [[ " $* " != *" -allowed-origins"* ]]; then
        export KC_ALLOWED_ORIGINS="${KC_ALLOWED_ORIGINS:-$DEFAULT_ORIGINS}"
        exec "#{libexec}/kc-agent-bin" -allowed-origins "${KC_ALLOWED_ORIGINS}" "$@"
      else
        exec "#{libexec}/kc-agent-bin" "$@"
      fi
    SHELL

    chmod 0755, bin/"kc-agent"
    bin.install_symlink bin/"kc-agent" => "bluefin-kubestellar"
  end

  service do
    run [opt_bin/"kc-agent"]
